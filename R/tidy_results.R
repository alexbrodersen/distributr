tidy.gresults <- function(x, arg_grid=NULL, .reps=NULL){
  ...
  value_grid <- rep_grid[rep(1:nrow(rep_grid), nkeys), ]

  rownames(value_grid) <- NULL
  res <- dplyr::as_data_frame(value_grid) %>% dplyr::bind_cols(., dplyr::bind_rows(val))
...
stack_list <- function(xl){
  ...
  if(is.data.frame(x)){
    ...
    if(same_dimensions){
      value <- xl %>% bind_rows %>% gather
      if(nrow(x) > 1){
        value <- value %>% transmute(key = key,
    ...
    if(same_dimensions){
      value <- do.call(rbind, xl) %>% as.data.frame %>% gather
    } else{
      value <- lapply(xl, stack_x) %>% bind_rows
    }
  }
...
stack_x <- function(x){
  if(is.data.frame(x)){
    if(nrow(x) > 1){
      ...
      data_frame(key = names(x), value = unlist(x, use.names = F))
    }
  } else {
    if(is.null(names(x))){
